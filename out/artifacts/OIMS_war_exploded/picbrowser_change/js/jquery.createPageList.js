...
(function($) {
	...
	/*
	 * current表示分页div对象 settings参数的集合
	 */
	...
	/*
	 * 显示list current当前div对象 data 是后台返回的一个结果集 settings前台封装的参数
	 */
	function showMyList(current, data, settings) {
		...
		if (formId == undefined)
			formId = "pageForm";// 默认值为pageForm
		var pageForm = $("#" + formId);// 是一个form对象
		if (!pageForm.length) {// form对象为空创建此form对象
			pageForm = $("<form/>").attr("id", formId).css("text-align",
					"right").width($(current).width()).appendTo(current);
		} else {// form对象不为空
			$(pageForm).text("");
		}
		$.each(settings.data, function(n, v) {
			$("<input/>").attr("name", n).attr("type", "hidden").val(v)
					.appendTo(pageForm);
		});
	...
	function addPageData(page, target) {
		...
		if (page.currentPage < page.pageCount) {
			...
			var l = $("<a href='#'>尾页</a>").appendTo(div);
			$(l).click(function() {
				$("input[name=currentPage]").val(page.pageCount);
				$(target).submit();
			});
			
			var nextPage = $("<a href='#'>下页</a>").appendTo(div);
		...
		if (page.currentPage > 1) {
			...
			var p = $("<a href='#'>上页</a>").appendTo(div);
			$(p).click(function() {
				var curPage = page.currentPage - 1;
				$("input[name=currentPage]").val(curPage);
				$(target).submit();
			...
			var firstPage = $("<a href='#'>首页</a>").appendTo(div);
		...
		var str = "<span>当前：第" + page.currentPage + "页&nbsp;共" + page.pageCount
				+ "页&nbsp;共" + page.rowsCount + "条&nbsp;&nbsp; &nbsp;</span>";
		$(str).appendTo(div);

	}

	/*
	 * current表示当前分页div对象 ListData后台返回的对象集合对象 settings前台封装的参数集合 根据数据集集合，动态打印表格
	 */
	function addListData(currentDiv, listData, settings) {
		var listObj = settings.listObj;// 表格标题的参数（对象的数组）listObj : [ {title :
		// "列标题一",key : ""},{title : "列标题二",key
		// : ""} ]
		var tableId = $(currentDiv).attr("id") + "Table";// 声明一个ID
		var tablex = $("<table/>").attr("id", tableId).attr("cellspacing", 0)
				.attr("cellpadding", 0).appendTo(currentDiv);// 创建一个table写入到currentDiv
		var trTitle = $("<tr/>").appendTo(tablex);// 表格第一行对象
		if (settings.checkbox == true)// true表示显示复选框
			$("<th/>").text("选择").appendTo(trTitle);// <th>则表示标题,一般用在一列的第一格,里面的内容会自动加粗加黑他们
		// 表格标题的打印
		$.each(listObj, function(i, obj) {
			$("<th/>").text(obj.title).appendTo(trTitle);// <th>则表示标题,一般用在一列的第一格,里面的内容会自动加粗加黑他们
		});
		if (settings.manageMenu != undefined && settings.manageMenu != null
				&& settings.manageMenu != "")
			$("<th/>").text("用户操作").appendTo(trTitle);
		...
		if (listData != null && listData.length != 0) {
							...
							function(m, data) {// 对象的集合
								...
								if (settings.checkbox == true)// true表示显示复选框,主要解决没有复选框单击行报错问题
								{
									trData
											.bind(
													'click',
													function() { // 创建一行写入到tablex表格中
														if (settings.single == true)// single==true表示复选框只能单选
														{
															var checkBoxObjm = $('#checkBoxObj'
																	+ m)[0];
															var state = false;
								...
								if (settings.checkbox == true)// true表示显示复选框
								{
									var checkBoxObj = $("<input />")
											.attr("type", "checkbox")
											.attr("name", "checkBoxObj")
											.attr("id", "checkBoxObj" + m)
											.attr("value", data)
											.attr(
													"value",
													(JSON.stringify(data)
									...
									$("<td />").html(checkBoxObj).appendTo(
											trData);
								}
								$
										.each(
												listObj,
												function(n, obj) {// key的集合
													var val_td = "-";
													$
															.each(
																	data,
								...
								if (settings.manageMenu != undefined
										&& settings.manageMenu != null
										&& settings.manageMenu != "")
									$(getManageMenuForList(data, settings))
											.appendTo(
													$("<td/>").appendTo(trData));
							});// 一层循环结束
			// 数据打印方法结束
			...
			var tdError = $("<td/>").html(tdErrorHtml).attr("rowspan",
					rowspanNumber).attr("colspan", colspanNumber).appendTo(
					trError);
	...
	function getManageMenuForList(data, settings) {
		...
		// manageMenu是一个操作的集合manageMenu : [{title : "操作一",url : "javascript:#"},
		// {title : "操作二",url : "javascript:#"} ]
		$.each(settings.manageMenu, function(i, menu) {
			...
			if (urlStr.split(":")[0] == "javascript") {
				...
				{
					var pamameters = urlStr.substring(begin + 1,
							urlStr.indexOf(")")).split(",");// 参数的数组
					for (var n = 0; n < pamameters.length; n++) {
						$.each(data,
								function(key, value) {
									if (pamameters[n] == key)
										urlStr = urlStr.replace(key, "'"
												+ value + "'");// 替换
								});
					}
					// alert(urlStr);
 ...
 */
function selectAllCheckboxByID(state, name, id, methodName) {
	var ids = document.getElementsByName(name);
	for (var i = 0; i < ids.length; i++) {
		if (ids[i].id == id) {
			ids[i].checked = state;
			if (state == true && methodName != null && methodName != "")
				methodName();
		} else
			ids[i].checked = false;
	}
...
/**
 * checkbox:是否选择checkBox
 * listObj:表格标题的参数
 * manageMenu：用户操作
 * single：复选框是不是单选
 */
//var setting{checkbox:true|false,};