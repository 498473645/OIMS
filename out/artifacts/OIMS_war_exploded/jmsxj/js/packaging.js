...
//不良反应
function setSgUnnormal(data){
	if(data!=null){
		$("#unnormal_id").val(data.id);
		$("#jlrq").val(formatDateDIY(data.jlsj,"-"));
		$("#blbh_bh").val(data.blbh);
		$("#csod").val(data.param_od);
		$("#csos").val(data.param_os);
		$("#pjsj").val(formatDateDIY(data.pjsj,"-"));
		$.each($("input[name='jcpd']"),function(i,el){
			if($(this).val() == data.jcpd){
				$(this).attr("checked","checked");
			}
		});
		$("#pinpai").val(data.pinpai);
		$("#fzren").val(data.fzren);
		$("#fssj").val(formatDateDIY(data.fssj,"-"));
		$.each($("input[name='yanbie']"),function(){
			if(data.yanbie.indexOf($(this).val()) != -1){
				$(this).attr("checked","checked");
			}
		});
		
		$.each($("input[name='yiji']"),function(){
			if(data.yiji.indexOf($(this).val()) != -1){
				$(this).attr("checked","checked");
			}
		});
		
		$.each($("input[name='erji']"),function(){
			if(data.erji.indexOf($(this).val()) != -1){
				$(this).attr("checked","checked");
			}
		});
		
		$.each($("input[name='sanji']"),function(){
			if(data.sanji.indexOf($(this).val()) != -1){
				$(this).attr("checked","checked");
			}
		});
		
		$.each($("input[name='clfs']"),function(){
			if(data.clfs.indexOf($(this).val()) != -1){
				$(this).attr("checked","checked");
			}
		});
		
		$("#yongyao").val(data.yongyao);
	}
}
//取镜单
function getQjd(){
...
function getSgBlfy(){
	...
	temp.blh = getBlh();
	temp.bl_id = $("#blbh").text();
	temp.id = $("#unnormal_id").val() == '' ? null : $("#unnormal_id").val();
	var jlsj = $("#jlrq").val();
	if(jlsj==''){
		jlsj = getDateNull();
	}else{
		jlsj = new Date($("#jlrq").val());
	}
	temp.jlsj = jlsj;
	temp.blbh = $("#blbh_bh").val();
	temp.param_od = $("#csod").val();
	temp.param_os = $("#csos").val();
	var pjsj = $("#pjsj").val();
	if(pjsj==''){
		pjsj = getDateNull();
	}else{
		pjsj = new Date($("#pjsj").val());
	}
	temp.pjsj = pjsj;
	var jcpd = "";
	$.each($("input[name='jcpd']"),function(){
		if($(this).attr("checked") == "checked"){
			jcpd += $(this).val();
		}
	});
	temp.jcpd = jcpd;
	temp.pinpai = $("#pinpai").val();
	temp.fzren = $("#fzren").val();
	var fssj = $("#fssj").val();
	if(fssj==''){
		fssj = getDateNull();
	}else{
		fssj = new Date($("#fssj").val());
	}
	temp.fssj = fssj;
	var yanbie = "";
	$.each($("input[name='yanbie']"),function(){
