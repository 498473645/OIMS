...
function set_jbxx(){
	...
	}else if(flag_text=="不良反应"){
		$("#show_moban").find("#suffererName").text(jbxx.name);
		$("#show_moban").find("#sex").text(jbxx.sex);
		$("#show_moban").find("#phoneNumber").text(jbxx.dianhua);
		$("#show_moban").find("#age").text(jbxx.age);
	}
}
function getUser(){
	...
	if(flag_text=="病历"){
		...
		if($("#blbh").text() != ''){
			...
			$("#pageListTable td").bind("click",function(){
				...
				if($(cb).attr("checked")!="checked"){
					var id = $(this).parent().children("td").eq(1).text();
					var data1 = getJSONData("/publish/shiGuang/getSgBlfyById.htm?id="+id,{}).obj;
					if(data1!=null){
						setSgUnnormal(data1);
					}
				}else{
...
function ti_unnormal(){
	var url_blfy = "";
	if($("#unnormal_id").val() == ""){
		url_blfy = "/publish/shiGuang/saveSgBlfy.htm";
	}else{
		url_blfy = "/publish/shiGuang/updateSgBlfy.htm";
	}
	var tmp = getSgBlfy();
	$.ajax({
		url : contextPath + url_blfy,
		data : tmp,
		type : "POST",
		dataType : 'json',
		success : function(data) {
...
function blfyList(blbh){
	var listFactor = {
			...
			url : contextPath + "/publish/shiGuang/findSgBlfy4page.htm?bl_id="+blbh,
			method : "post",
			checkbox : true,
			single : false,
			data : {// data表示传的参数
