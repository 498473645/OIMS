...
//不良反应
function sg_unnormal(){
	$("#tabContent").html("");
	var show_moban = $("<div/>").attr("id","show_moban").html(blmb1(380));
	$("#tabContent").append(show_moban);
	set_values();//赋值
	//加载功能按钮
	var funButtons = $("<div/>").attr("style","width:100%;height:20px;text-align:center;margin-top:10px;").html("" +
			"<input type='hidden' id='unnormal_id' style='width:50px;'/>&nbsp;&nbsp;"+
			"<input type='button' value='提交' style='width:60px;height:20px;' onclick='ti_unnormal()'/>&nbsp;&nbsp;" +
			"<input type='button' value='重置' style='width:60px;height:20px;' onclick='sg_unnormal()'/>&nbsp;&nbsp;" +
			"<input type='button' value='打印' style='width:60px;height:20px;' onclick='print_unnomal()'/>");
	$("#tabContent").append(funButtons);
	
	calendarFun_sg("jlrq");
	calendarFun_sg("pjsj");
	calendarFun_sg("fssj");
	checkbox_single("jcpd");
