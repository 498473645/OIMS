...
function initBtnFooter(){
	...
	$("<a href='javascript:saveVision();'><span class='save'></span>"+xiaoping_language.Save+"</a>").appendTo("#footer");
...
function showJcdForm(){
	...
	$("<div/>").addClass("title1").text(xiaoping_language.shilijiancha).appendTo(".left");
	$("<div/>").attr("id","yalitab").addClass("tabmain1 list").appendTo(".left");
	var shilitab = "<form id='shiliFm'><table width='100%' border='0' cellSpacing='0' cellPadding='0'>" +
	               "<tr><th></th><th>"+xiaoping_language.LuoYanShiLi+"</th><th>"+xiaoping_language.JiaoZhengShiLi+"</th><th>"+xiaoping_language.JinShiLi+"</th></tr>"+
	               "<tr><td class='seet' nowrap='nowrap'>"+xiaoping_language.LeftEye+"</td>" +
	                   "<td><input name='nvLeft' id='nvLeft' class='seeing' type='text' /></td>" +
	                   "<td><input name='corLeft' id='corLeft' class='seeing' type='text' /></td>" +
	                   "<td>" +
	                   "<input name='nearLeft' id='nearLeft' class='seeing' type='text' />" +
	                   "<input type='hidden' id='jzId' name='ghid'/>" +
	                   ...
	                   "</td></tr>"+
	               "<tr><td class='seet' nowrap='nowrap'>"+xiaoping_language.RightEye+"</td>" +
	                    "<td><input name='nvRight' id='nvRight' class='seeing' type='text' /></td>" +
	                    "<td><input name='corRight' id='corRight' class='seeing' type='text' /></td>" +
                       "<td><input name='nearRight' id='nearRight' class='seeing' type='text' /></td></tr>"+
			       "</table></form>";
	$(shilitab).appendTo("#yalitab");
	
	$(".seeing").click(function(e){
		var event = window.event || e;
		var taget = event.srcElement || event.target;
		focusId = taget.id;
	});
	
	var ul = "<ul id='shiliUl'>" +
	...
	$("#shiliUl li").click(function(e){	
		...
		if(val=='c'){
			...
			if(prevalue.indexOf(".")!=-1){
				$.oimsAlert(xiaoping_language.shilishurubuzhengque+"!");
				return;
			}
	    	$("#"+focusId).val(prevalue+val);
		}else{
			var prevalue = $("#"+focusId).val();
			var shili = prevalue+val;
			if(!isShiLiValidate(shili)){
				$.oimsAlert(xiaoping_language.shilishurubuzhengque+"!");
				return;
			}
			$("#"+focusId).val(prevalue+val);
...
function isShiLiValidate(p){
	if(p==undefined||p=="") return true ;
	var partten1 = /^(4)((\.)[0-9]{1})?$/;
	var partten2 = /^(5)((\.)[0-3]{1})?$/;
	if(p==0||p==0.1||p==0.12||p==0.15||p==0.2
		||p==0.25||p==0.3||p==0.4
		||p==0.5||p==0.6||p==0.8
		||p==1.0||p==1.2||p==1.5||p==2.0){
		return true;
	}
	if(partten1.test(p)||partten2.test(p)) {   
	       return true;   
	}   
	   return false;
}
...
function saveVision(){
	if(isVisionConfirm()){
		$("#shiliFm").ajaxForm( {
			url: contextPath + "/publish/ShiLi/visionSave.htm",
			type: 'post',
	        dataType : 'json',
...
function isVisionConfirm(){
	var nvleft = $("#nvLeft").val();
	var corleft = $("#corLeft").val();
	var nearleft = $("#nearLeft").val();
	var nvright = $("#nvRight").val();
	var corright = $("#corRight").val();
	var nearright = $("#nearRight").val();
	...
	if(nvleft==""){
	   $.oimsAlert(xiaoping_language.shuruzuoyanluoyan);
	   return false;
	}
	if(corleft==""){
		$.oimsAlert(xiaoping_language.shuruzuoyanjiaozheng);
	   return false;
    }
	if(nearleft==""){
		$.oimsAlert(xiaoping_language.shuruzuoyanjinshili);
	   return false;
	} 
	if(nvright==""){
		$.oimsAlert(xiaoping_language.shuruyouyanluoyan);
	   return false;
	}
		
    if(corright==""){
    	$.oimsAlert( xiaoping_language.shuruyouyanjiaozheng);
	   return false;
	}
	if(nearright==""){
		$.oimsAlert(xiaoping_language.shuruyouyanjinshi);
	   return false;
	}   
	  return true;
}
...
//		showExecuteJcdList();
		$.oimsSucc(xiaoping_language.shilibaocunchenggong);
		
	}else{
		$.oimsError(xiaoping_language.shilibaocunshibai);
	}
}
