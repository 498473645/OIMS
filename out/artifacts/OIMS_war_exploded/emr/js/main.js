...
var SAVE_PATIENT_DIQU_URL='/publish/huanZheXinXi/updateHzxxDiqu.htm';
var winWidth;
var winHeight;
var currentVisit;
var currentDoctor;
...
//	});
	//console.dir(document.getElementById('careFrame').contentWindow.document);
//	$(document.getElementById('careFrame').contentWindow.document).ready(function(){
//		debugger;
//		document.getElementById('careFrame').contentWindow.document.onkeypress=banBackSpace1
//	    //禁止退格键 作用于IE、Chrome
//		document.getElementById('careFrame').contentWindow.document.onkeydown = banBackSpace1;
//		function banBackSpace1(){
//			debugger;
//			alert(1);
//		}
//	});
	    ...
	    function banBackSpace(e){
	        ...
	        var flag2 = ev.keyCode == 8 && t != "password" && t != "text" && t != "textarea"&&(obj.contentEditable!="true");
	        //判断
	        if (flag2 || flag1)
...
function showPatientVisits(){
	...
	var age = _emr_calculteAge(formatDate((hzxx.shengri.time)));
	...
	if(!$("#mobile").text()){
		...
		importJS("/js/diqu.js");
		var tag = $("<div />").css("display","inline").appendTo("#patientInfo")
		showFullDiquSelect(tag,hzxx.diquId,function(i){
			saveOrUpdatePatientDiqu(hzxx.id,i);
		});
	showWorkStation(data.obj.visitList);
}

function saveOrUpdatePatientDiqu(id,i){
	var re = getJSONData(SAVE_PATIENT_DIQU_URL, {patientId:id,diquId:i},"POST");
	if(!re.state){
		$.oimsError("更新患者地区信息失败！");
	}else{
		$.oimsAlert("患者地区成功更新！");
	}
}
...
function showMedicalRecord(visit){
	...
	var now = new Date();
	if(fucha){
		if((formatDate(visitDate)==formatDate(now))||fenzhenkaidan==1){
	...
	$("#printBingLi").unbind('click').click(function(){
		...
		$("#printch").click(function(){
			...
			function printEMRB5_JIWANG(){
				...
				var data = getJSONData(EMR_PATIENT_INFO_URL,{binglihao:currentPatient.binglihao},"POST");
				...
				var shenriFlag = ""
				if (visit.caozuoTime != null) {
					var year = parseInt(formatDate(new Date()).substring(0, 4)) - parseInt(formatDate(visit.caozuoTime.time).substring(0,4))//获取历史时间和当前时间相差多少年
				...
				showWithHtmlTemplate(
						"print_prescribe_xiyaofang",
						{
							...
							sex : sex,
							age : age,
							kdsj : formatDate(currentVisit.caozuoTime.time),
							kdys :doctor_name,
							zhenduan : zd,
							keshi : "眼科门诊"
						}, page);
		...
		$("a#printbl").click(function(){
			printEMRB5_JIWANG();
		});

		function printEMRB5_JIWANG(){
			//var EMR_PATIENT_INFO_URL = "/publish/emr/showDoctorWorkstation.htm";//为何要加这行？
			var CATEGORY_GET_URL = "/publish/category/getCategoryById.htm";
			var GETPRINTDATA_URL="/publish/emr/getprintdata.htm";
			...
			var age = _emr_calculteAge(formatDate(shenriFlag!=""?shenriFlag:currentPatient.shengri.time));
			var doctor_name=getJSONData(getDoctorNameUrl,{gonghao:currentVisit.fzys},'POST').obj;
			showWithHtmlTemplate("emr_print",{binglihao:currentPatient.binglihao,xingming:currentPatient.xingming,sex:sex,age:age,kdsj:formatDate(currentVisit.caozuoTime.time),kdys:doctor_name},page);
			showBarcode(currentPatient.binglihao,page.find(".barcodeDiv"));
			$('.printPage h1').css({'line-height':'50px'});
			var tag = page.find(".orderList");
			$.each(printData.records,function(){
				if(this.categoryId==30100){
					...
					$("<p />").addClass("p_emr_zhusu").text(this.jilu).appendTo(div);
				}
				if(this.categoryId==30102){
					if(this.jilu){
						div = $("<div />").appendTo(tag);
			...
			p0=$("<p />").addClass("zhenduan_emr").appendTo(div);
			...
			function showCHUZHI(printData,p0){
				...
				var sf=printData.sf;
				chuzhi+=(lis?(lis+";"):'')+(hospital?(hospital+";"):'')+(special?(special+";"):'')+(prescription?(prescription+";"):'')+(treat?(treat+";"):'')+(st?(st+";"):'')+(sf?((sf.yyrq?('<br />复诊时间:'+formatDate(sf.yyrq.time)+'。'):'')+(sf.zhuyi?('<br />'+sf.zhuyi+'。'):'')):'');
				p0.html(chuzhi);
			}
...
function _emr_operation(id){
	var tabData = getTabData(id);
	if(tabData==null)return;
	var tag1=tabData.tag1;
	var tag2=tabData.tag2;
	//TODO 查询手术记录
	var data=getJSONData('/publish/shoushu/getOperationList.htm',{patientId:currentPatient.binglihao,tag:Math.random()},'POST').obj;
	if(data&&data.length!=0){//operationTime//operationRecord
		for(var i=0;i<data.length;i++){
			var div=$('<div style="float:left;margin:5px;">').appendTo(tag2);
			div.append('<input type="button" value="'+formatDate(data[i].operationTime.time)+'" style="width:100px;height:25px"/>');
			div.data('data',data[i]).click(function(){
				$("<div />").oimsDialog({
					title : "手术记录",
					width : 900,
					icon : 'view',
				...
				$('div.openWin').find('.opencontent').text($(this).data('data').operationRecord?$(this).data('data').operationRecord:'');
			});
		}
	}
	
}
function _emr_jiaomoyizhi(id){
	debugger;
	...
	view_jmyz(id,currentPatient);
}

function _emr_treat_result(id){
	var tabData = getTabData(id);
	if(tabData==null)return;
	var tag1=tabData.tag1;
	var tag2=tabData.tag2;
	var data=getJSONData('/publish/jcd/getTreatResult.htm',{patientId:currentPatient.binglihao},'POST').obj;
	var div1_tag2=$("<div style='width:100%;height:"+tag2.height()/2+"px;'>").appendTo(tag2).css({'border-bottom-style':'groove','border-bottom':'black'});
	var div2_tag2=$("<div style='width:100%;height:"+tag2.height()/2+"px;'>").appendTo(tag2);
	if(data.ldcx&&data.ldcx.length){
		$("<div style='width:100%'>").text('泪道冲洗').css({'font-size':'x-large','margin-left':'5px'}).appendTo(div1_tag2);
		var div1_tag2_content=$("<div style='width:100%'>").appendTo(div1_tag2);
		for(var i=0;i<data.ldcx.length;i++){
			var div=$('<div style="float:left;margin:5px;">').appendTo(div1_tag2_content);
			div.append('<input type="button" value="'+data.ldcx[i].cli_date+'" style="width:100px;height:25px"/>');
			div.data('data',data.ldcx[i]).click(function(){
				$("<div />").oimsDialog({
					title : "泪道冲洗",
					width : 900,
					icon : 'view',
				...
				$('div.openWin').find('.opencontent').text($(this).data('data').demo?$(this).data('data').demo:'').css({'font-size':'25px'});
			});
		}
		}
	if(data.jgzl&&data.jgzl.length){
		$("<div style='width:100%'>").text('激光治疗').css({'font-size':'x-large','margin-left':'5px'}).appendTo(div2_tag2);
		var div2_tag2_content=$("<div style='width:100%'>").appendTo(div2_tag2);
		for(var i=0;i<data.jgzl.length;i++){
			var div=$('<div style="float:left;margin:5px;">').appendTo(div2_tag2_content);
			div.append('<input type="button" value="'+data.jgzl[i].cliDate+'" style="width:100px;height:25px"/>');
			div.data('data',data.jgzl[i]).click(function(){
				$("<div />").oimsDialog({
					title : $(this).data('data').type,
					width : 900,
					icon : 'view',
					height : 500,
					drag : false,
				...
				$('div.openWin').find('.opencontent').text($(this).data('data').result?$(this).data('data').result:'').css({'font-size':'25px'});
			});
		}
		}
...
//			if(d.categoryid==oimsCategory.WENZHENMOBAN_ZHUSHU){
		...
		success : function(data) {
			if(data.state&& data.obj!=null){
				...
				if(data.obj.picPath!=null && data.obj.picPath.length){
				 ...
				 if(d.categoryid == "30102" || d.categoryid == "30103" || d.categoryid == "30104" || d.categoryid == "30105"){
						div.html("无特殊");
				  }
			}
