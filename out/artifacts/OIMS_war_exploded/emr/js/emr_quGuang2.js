...
function view_qg(id){
	...
	if(tag.length){
		...
		var tag4 = $("<div/>").attr("id","div_show_4").attr("style","width:"+(qgblyl_width)+"px;height:"+(showHeight-2)+"px;background:;").appendTo("#medicalRecordTab"+id);
		//var tag4_1 = $("<div/>").attr("id","div_show_4_1").attr("style","width:20px;height:"+(h-20)+"px;background:;float:left;").appendTo(tag4);
		var tag4_2 = $("<div/>").attr("id","div_show_4_2").attr("style","width:"+(qgblyl_width-43)+"px;height:"+(showHeight-2)+"px;background;float:left;border-top:1px solid #d2d2d2;border-left:1px solid #d2d2d2;border-right:1px solid #d2d2d2;").appendTo(tag4);
		...
		var tag4_2_2 = $("<div/>").attr("id","div_show_4_2_2").attr("style","width:"+(qgblyl_width-40)+"px;height:"+(showHeight-30-28)+"px;background:;overflow:auto;").appendTo(tag4_2);
		//读病例模板
//Liyan		_emr_show_bl();
		/*var blh = $("#patientInfo").children("span").first().text();
		//赋值——患者基本信息
...
function ssjl_set_jbxx(blh){
	$("#ssjl_show_div").find("#caseNumber").text(currentPatient.binglihao);
	$("#ssjl_show_div").find("#suffererName").text(currentPatient.xingming);
	$("#ssjl_show_div").find("#sex").text(currentPatient.xingbie?"男":"女");
	$("#ssjl_show_div").find("#age").text(currentPatient.age);
}
function shjl_set_jbxx(blh){
	$("#shjl_show_div").find("#caseNumber").text(currentPatient.binglihao);
	$("#shjl_show_div").find("#suffererName").text(currentPatient.xingming);
	$("#shjl_show_div").find("#sex").text(currentPatient.xingbie?"男":"女");
	$("#shjl_show_div").find("#age").text(currentPatient.age);
}
//病历赋值——患者基本信息
function bl_set_jbxx(blh){
	console.log("---------------患者基本信息------------------");
	console.log(currentPatient);
	$("#show_bl").find("#caseNumber").text(currentPatient.binglihao);
	$("#show_bl").find("#qqNumber").val(currentPatient.qq);
	$("#show_bl").find("#suffererName").text(currentPatient.xingming);
	$("#show_bl").find("#sex").text(currentPatient.xingbie?"男":"女");
	$("#show_bl").find("#birthday").text(formatDate(currentPatient.shengri.time));
	$("#show_bl").find("#job").val(currentPatient.job);
	$("#show_bl").find("#address").val(currentPatient.jtdz);
	$("#show_bl").find("#cellphone").val(currentPatient.shouji?currentPatient.shouji:currentPatient.dianhua);
	$("#show_bl").find("#age").text(currentPatient.age);
	/*var jbxx = getJbxx_qg(blh);
...
//	}
	...
	var temp = {
			...
			ssfy:$("#ssfy").val(),
			qg_jcf:$("#qg_jcf").val(),
			ssf_rq:$("#ssf_rq").val()==''?(new Date(null)):(new Date($("#ssf_rq").val())),
			jcf_rq:$("#jcf_rq").val()==''?(new Date(null)):(new Date($("#jcf_rq").val()))
			}
	return temp;
}
//病例取值
...
function tbl_yy_add(){
	...
	var tab_add = "<table id='ssjlAdd' cellpadding='0' cellspacing='0' width='100%' style='border-top:1px solid rgb(51,51,51);border-left:1px solid rgb(51,51,51);'>" +
					...
					"<td align='center' class='td_border' colspan='4' align='center'>" +
						"<input type='checkbox' id='yyxm2' name='yyxm'/>&nbsp;&nbsp;LASEK 系列&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
				...
				"<tr>"+
				"<td align='center' class='td_border' colspan='3' align='center'>" +
					"<input type='checkbox' id='yyxm5' name='yyxm'/>&nbsp;&nbsp;Smart-TPRK&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
					"&nbsp;&nbsp;或&nbsp;&nbsp;<input type='text' id='ssfy5h' style='width:50px;'/>  元"+
				"</td>" +
				"<td align='center' class='td_border' colspan='4' align='center'>" +
					"<input type='checkbox' id='yyxm6' name='yyxm'/>&nbsp;&nbsp;全飞秒&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
						"<input type='text' id='ssfy6h' style='width:50px;'/>  元"+
				"</td>" +
			"</tr>" +
				"<tr>" +
...
//	$("<div/>").attr("id","div_liebiao").attr("style","width:90%;height:20px;margin-left:5%;background:url(../style/green/images/thbg.png) repeat-x left;").appendTo(div_body).html("<h3>>>详情</h3>");
	var div_top = $("<div/>").attr("id","ssjl_add").attr("style","width:90%;height:1000px;margin-left:5%;background:;border:0px solid #d2d2d2;").appendTo(div_body);
	$("<div/>").attr("style","width:90%;height:20px;margin-left:5%;").appendTo(div_body);
//	$("<div/>").attr("style","width:90%;height:20px;margin-left:5%;background:url(../style/green/images/thbg.png) repeat-x left;").appendTo(div_body).html("<h3>>>列表</h3>");
//	var div_bottom = $("<div/>").attr("id","ssjl_list").attr("style","width:90%;height:170px;margin-left:5%;background:;border:0px solid #d2d2d2;").appendTo(div_body);
	...
	//基本信息赋值
	var patientId = currentPatient.binglihao;
	var jbxx = getJbxx_qg(patientId);
	$("#kebie").text("眼科屈光中心");
	$("#suffererName").text(jbxx.name);
	$("#blh").text(jbxx.blh);
...
function ti_jt_ssjl(autoSaveTag){
	if(!$("#kssj").val().length){
		$.oimsAlert("手术开始时间不能为空！");
		return;
	}
	if($("#kssj").val().length!="yyyy-MM-dd HH:mm".length){
		$.oimsAlert("手术开始时间格式应为:yyyy-MM-dd HH:mm");
		return;
	}
	if(!$("#jssj").val().length){
		$.oimsAlert("手术结束时间不能为空！");
		return;
	}
	if($("#jssj").val().length!="yyyy-MM-dd HH:mm".length){
		$.oimsAlert("手术结束时间格式应为:yyyy-MM-dd HH:mm");
		return;
	}
	if($("#sel_lev1").val()==""){
		$.oimsAlert("请选择手术方式！")
...
function jtss_update_set(data){
	...
	$("#qt").val(data.qt);
	var kssj = formatDateTime(data.kssj.time);
	if(kssj.length>1)kssj = kssj.substring(0,kssj.length-3);
	$("#kssj").val(kssj);
	var jssj = formatDateTime(data.jssj.time);
	if(jssj.length>1)jssj = jssj.substring(0,jssj.length-3);
	$("#jssj").val(jssj);
...
function er_ssjl_get(){
	...
	var czrq = new Date();
	var ssrq_r = $("#ssrq_r").val()==""?new Date(null):new Date($("#ssrq_r").val());
	var ssrq_l = $("#ssrq_l").val()==""?new Date(null):new Date($("#ssrq_l").val());
	var temp = {
			id:er_id,
			blh:blh,
			zlfs_r:$("#zlfs_r").val(),
...
function reset_ssjl_jt_top(){
	...
	$("<div/>").attr("style","width:100%;height:20px;text-align:center;margin-top:0px;").html("" +
			...
			"<input type='button' value='重置' style='width:60px;height:20px;' onclick='_emr_show_ssjl();'/>&nbsp;&nbsp;&nbsp;&nbsp;" +
			"<input type='button' value='打印' style='width:60px;height:20px;' onclick='print_jtssjl()'/>").appendTo(div_top);
//		calendarFun_yy("ssrq_l");
//		calendarFun_yy("ssrq_r");
}

function addQGJTSSYanbie(){
	console.log("-------------------addQGJTSSYanbie--------------------------")
	var div = $("<div />").attr("id","JTSLJLYanbieDiv").width(623).css("margin","auto").height(22).insertBefore("#ssjl_add");
	var od = $("<a />").text("右眼").css({display:"block",width:"40px", "text-align":"center", height:"22px", "line-height":"22px",float:"left"}).click(function(){
		if(od.hasClass("on"))return;
		os.removeClass("on").css({"border":0,"color":"#000", "background":"none"});
		od.addClass("on").css({"border-color":"red", "color":"#fff", "background":"red"});
		reset_ssjl_jt_top();
		setValue_qg();
		$("#yanbie").val("r");
		$("#ssjl_add").find("h1").text($("#ssjl_add").find("h1").text()+"（右眼）");
		//var jbxx = getJbxx_qg(patientId);
		$("#kebie").text("眼科屈光中心");
		$("#suffererName").text(currentPatient.xingming);
		$("#blh").text(currentPatient.binglihao);
	}).appendTo(div);
	var os = $("<a />").text("左眼").css({display:"block",width:"40px", "text-align":"center", height:"22px", "line-height":"22px",float:"left"}).click(function(){
		if(os.hasClass("on"))return;
		od.removeClass("on").css({"border":0,"color":"#000", "background":"none"});
		os.addClass("on").css({"border-color":"red", "color":"#fff", "background":"red"});
		reset_ssjl_jt_top();
		setValue_qg();
		$("#ssjl_add").find("h1").text($("#ssjl_add").find("h1").text()+"（左眼）");
		$("#yanbie").val("l");
		$("#kebie").text("眼科屈光中心");
		$("#suffererName").text(currentPatient.xingming);
		$("#blh").text(currentPatient.binglihao);
...
function ssjl_get(){
	...
	var ssys_l = $("#ssys_l_gh").val();	//手术医生 左眼
	var ssys_r = $("#ssys_r_gh").val();	//手术医生 右眼
	...
	var ssrq_l = new Date(null);
	if($("#ssrq_l").val()!=''){
		ssrq_l = new Date($("#ssrq_l").val());
	}
	var ssrq_r = new Date(null);
	if($("#ssrq_r").val()!=''){
...
function ssfsjl(){
	...
	var sel_fl1 = "<select id='sel_lev1' onchange='fenlei_lev2();'>" +
			...
			"<option value='准分子'>准分子</option>" +
			"<option value='晶体植入'>晶体植入</option>" +
			"<option value='儿童屈光'>儿童屈光</option>" +
			"</select>";
...
function fenlei_lev2(){
	//改变手术方式，刷新病历页面
	var v = $("#sel_lev1").val();
	var fenlei = parseInt(v);
	console.log("v:"+v);
	if(fenlei=='准分子'){
		var sel_fl2 = "<select id='sel_lev2' onchange='fenlei_lev3();'>" +
							"<option value='lasik-110'>lasik-110</option>" +
							"<option value='lasik-90'>lasik-90</option>" +
							"<option value='lasik飞秒'>lasik飞秒</option>" +
							"<option value='PRK'>PRK</option>" +
							"<option value='Smart-TPRK'>Smart-TPRK</option>" +
							"<option value='全飞秒'>全飞秒</option>" +
						"</select>";
...
function ti_blEr_qg(autoSaveTag){
	...
	if($("#sel_lev1").val()==""){
		...
		if(blbh==""){
			...
			$("#bingliNumber").val(blbh);
		}else{
			if(isNaN(blbh)){
				$.oimsAlert("您输入的病历编号不是数字，病历编号的格式为：4位数字年+4位数字尾号，如：20150001。请您按照规定格式填写，或者不填写，由系统自动生成！");
				return false;
			}else{
				if(blbh.length<8){
					$.oimsAlert("您输入的病历编号位数不是8位，病历编号的格式为：4位数字年+4位数字尾号，如：20150001。请您按照规定格式填写，或者不填写，由系统自动生成！");
					return false;
				}
			}
		}
...
function ti_bl_qg(autoSaveTag){
	...
	if($("#sel_lev1").val()==""){
		...
		temp.bingliNumber = blbh;
		var re = getJSONData(url_bl,temp,"POST");
		if(re.state){
			if($("#lc_id").text()=="无")qglc_get();
			if(auto_fx_zfz_qg()){
				$.oimsSucc('操作成功！');
			}else{
				$.oimsAlert('操作失败！');
			...
			"<table id='ssjlAdd' cellpadding='2' cellspacing='0' width='100%'>" +
				...
				"<table id='ssjlAdd' cellpadding='2' cellspacing='0' width='100%' style='font-size:14px;'>" +
					...
					area+="</label>"+
						...
						"<td style='font-size:18px;'>" +
							"LASEK 系列：" +
						"</td>" +
						"<td style='border-bottom:1px solid black;font-size:16px;' align='center'>";
							if(temp.yyxm2=="1"){
								area+=ssfy2;
							}
				area+="</td>" +
				...
				"<table id='ssjlAdd' cellpadding='5' cellspacing='0' width='100%' >" +
					"<tr>" +
						...
						area+="</span>&nbsp;&nbsp;" +
							"<span style='font-size:18px;'>ICL</span>" +
							"<span style='border-bottom:1px solid black;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label id='icl'>";
								if(temp.yyxm4=="1"){
									area+=ssfy4;
								}
						area+="</label>&nbsp;&nbsp;&nbsp;&nbsp;</span>" +
							"（晶体植入手术，适合高度近视、角膜扁薄或形态异常患者）" +
						"</td>" +
					"</tr>" +
					"<tr>" +
					"<td style='font-size:16px;'>" +
					...
					area+="</span>&nbsp;&nbsp;" +
						"<span style='font-size:18px;'>Smart-TPRK</span>" +
						"<span style='border-bottom:1px solid black;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label id='icl'>";
							if(temp.yyxm5=="1"){
								area+=ssfy5;
							}
				...
				area+="</span>&nbsp;&nbsp;" +
					"<span style='font-size:18px;'>全飞秒</span>" +
					"<span style='border-bottom:1px solid black;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label id='icl'>";
						if(temp.yyxm6=="1"){
							area+=ssfy6;
						}
				area+="</label>&nbsp;&nbsp;&nbsp;&nbsp;</span></br></br>" +
				"</td>" +
			"</tr>" +
					"<tr>" +
					...
					"<tr>" +
						...
						"</td>" +
					"</tr>" +
					"<tr><td style='font-size:16px;'>3.学生、教师及军人，请带上相关证件，享受优惠</td></tr>" +
...
function qglc_get(){
	...
	if(lc_id==null){
		temp.id = lc_id;
		temp.blh = currentPatient.binglihao;
		temp.state = 1;
...
//流程id
function qulc_set(){
	console.log("--------------------qulc_set---------------------------------")
	var data = getJSONData("/publish/quguang/findQglcByBlhWwc.htm",{blh:currentPatient.binglihao},"POST");
	var lc_name = "";
	if(data!=null){
		$("#lc_id").text(data.id);	
...
function getLastOneQgbl(fl){
	//病例号
	var blh = currentPatient.binglihao;
	var url = "";
	if(fl=='准分子'){
	...
	if(ssfs!=""){
		if(sel_tag=="病历"){
			...
			if(ssfs==1){
				...
				url=SSJL_JT;
				form = $.extend(form,{yanbie:$("#yanbie").val()});
			}else if(ssfs==3){
				url=SSJL_ER;
			}
		}else if(sel_tag=="术后复查"){
		...
		if(url!="" && lc_id!=null && lc_id!='无'){
			var data = getJSONData(url,form,"POST").obj;	
			if(data!=null){
				if(sel_tag=="病历"){
					if(ssfs=='准分子' || ssfs=='晶体植入'){
						reloadBl();
...
function getJiuzhen_qg(blh,th){
	...
	if(currentPatient!=null){
		var data = getJSONData(EMR_JIUZHEN_URL,{id:currentPatient.id}).obj;
		if(data!=null){	
			if(th=="last"){
...
function openFloatDiv(id){
	if($("#floatDiv").attr("id")==undefined){
		...
		if(id=="zs_l" || id=="zs_r"){
			...
			tab = floatDivChild3();
			$(floatDiv).append(tab);
		}else if(id=="ssys_l" || id=="ssys_r" || id=="zdys" || id=="zsys" || id=="hs" || id=="ptjs_r" || id=="ptjs_l" || id=="pths_r" || id=="pths_l"){
			$("#floatDiv").attr("style","width:"+(widthDiv-2)+"px;height:200px;z-index:9999;position:absolute;top:"+topPosition+"px;left:"+leftPosition+"px;border:1px solid #d2d2d2;overflow:auto;background:white;");
			//tab = floatDivChild4();
