...
var saveZkjcToHis='/publish/jzjl/saveZkjcToHis.htm';
var eyePDFBaoGao=[];
var item_class={'A':'西药','B':'中药','C':'化验','D':'检查','E':'治疗','F':'手术','G':'麻醉','H':'血费','I':'材料'};
var print_jcxm_differ=[1983,1984,1985,1986];
var qrcode_url='/publish/code/getQCodeImg.htm';
var hospital_hz='/publish/shoushu/isInHospital.htm';
var jifeiFlagPrint = false;
...
function printEMRB5(big){
	...
	var record_zhushu = $("#"+id).text();
	$("<p />").addClass("p_emr_zhusu").text(record_zhushu).appendTo(div);
	...
	if($("#"+id).text()){
		...
		$("<strong />").text("既往史：").appendTo(div);
		$("<p />").text($("#"+id).text()).appendTo(div);
		page=backPageCombineCurrentPage(page,"既往史：",win);
	}
	id = oimsCategory.WENZHENMOBAN_XIANBINGSHI+"_"+currentVisit.id;
	if($("#"+id).text()){
		div = $("<div />").appendTo(page.find('.orderList'));
		$("<strong />").text("现病史：").appendTo(div);
		$("<p />").text($("#"+id).text()).appendTo(div);
		page=backPageCombineCurrentPage(page,"现病史：",win);
	}

	div = $("<div />").appendTo(page.find('.orderList'));
	$("<strong />").text("专科检查：").appendTo(div);
	var p0 = $("<p />").addClass("zkjc_emr").appendTo(div);
	var txt=showTxt(getZKJCText,p0,_show);
	page=backPageCombineCurrentPage(page,"专科检查：",win,_show);

	div = $("<div />").appendTo(page.find('.orderList'));
	$("<strong />").text("诊断：").appendTo(div);
	var record_zhenduan=getZhenduanText();
	$("<p />").addClass("zhenduan_emr").text(record_zhenduan).appendTo(div);
...
function backPageCombineCurrentPage(page,title,win,_show){
	...
	if(height>=310){
		...
		if(title=='既往史：'){
			...
			var	div = $("<div />").appendTo(tag2);
			var	id = oimsCategory.WENZHENMOBAN_XIANBINGSHI+"_"+currentVisit.id;
			$("<strong />").text("现病史：").appendTo(div);
			$("<p />").text($("#"+id).text()).appendTo(div);
		}else if(title=='专科检查：'){
			var	div = $("<div />").appendTo(tag2);
			...
			var div = $("<div />").appendTo(tag2);
			$("<strong />").text("诊断：").appendTo(div);
			$("<p />").addClass("zhenduan_emr").text(getZhenduanText()).appendTo(div);
		}else if(title=='处置：'){
			var div = $("<div />").appendTo(tag2);
			$("<strong />").text("处置：").appendTo(div);
...
function printComboList(listTypes){
	...
	if(($(this).text()=='金额'&&listType!=CHUZHI_CATEGORY.Qg_operation)||$(this).text()=='库存'||$(this).text()=='药局'||$(this).text()=='生产厂商'){
			...
			if(listType==CHUZHI_CATEGORY.prescribe&&($(tr).find("td:eq(10)").text()=='便民药房'||$(tr).find("td:eq(10)").text()=='药品超市')){
				if(!t1.length){
					if(page1==null){
						...
						if(listType==CHUZHI_CATEGORY.otherExam){
								...
								if(bgsbm=='4102'||bgsbm=='410201'||bgsbm=='410202'||bgsbm=='410203'||bgsbm=='410204'||bgsbm=='410205'||bgsbm=='410206'||bgsbm=='410230'){
									if(!tag.find('div.fsknote').length){
										...
										div.append((zysx.scheduledDate?("请于"+formatDate(zysx.scheduledDate.time)+","):'')+(zysx.address?zysx.address:'')+"。").append("</br>放射科上下班时间为上午8:00-12:00，下午1:00-4:30,周末下午休息。");
									}
								}
				...
				$.each($('table.jcxmFushu'),function(i,n){
					if($(this).attr('id').split('_')[1]==jcxmId){
						$.each($(this).find('tr'),function(){
							...
							if(listType!=CHUZHI_CATEGORY.operation){
								if(t.find("tr").length>8||jcxmId==1000357||jcxmId==1000358){
						...
						//名称
						if($(this).index()==0){
							if((parseInt(jcxmId)>=1983&&parseInt(jcxmId)<=1984)||parseInt(jcxmId)==1985||parseInt(jcxmId)==1986||parseInt(jcxmId)==1000104||parseInt(jcxmId)==1000105||parseInt(jcxmId)==1000106||$.inArray(parseInt(jcxmId),print_jcxm_differ)!=-1){
								if(parseInt(jcxmId)==1000107){
									temp="眼科检查";
						...
						if(listType==CHUZHI_CATEGORY.operation){
							...
							td0.css({"text-align":"left","font-size":"15px"});
							td0.attr("nowrap","nowrap");
							td0.text((parseFloat($($(this).find('input:eq(0)')[0]).data('price'))*parseFloat($(this).find('td:eq(2)').find('input').val()*$(tr).find('input[name=count]').val())).toFixed(2));
						}else{
							if($(this).index()==0){
								//眼别
								td0=$("<td />").appendTo(tr0);
						...
						while(tr0.find("td").length<th.find("th").length){
							$("<td />").appendTo(tr0);
						}
					});
					$("<tr/>").append($('<td colspan="'+th.find("th").length+'" style="text-align: left; font-size: 15px;" nowrap="nowrap"/>').text('--------------------------------')).appendTo(t);
				}
			});
	...
	if(listType==CHUZHI_CATEGORY.otherExam)
		...
		showWithHtmlTemplate("print_labTest", print_data, page);
			...
			&& ($(tr).find("td:eq(10)").text() == '便民药房'||$(tr).find("td:eq(10)").text() == '药品超市')) {
		page.addClass("ypcs");
		showWithHtmlTemplate("print_prescribe_bianmin", print_data, page);
	} else if(listType == CHUZHI_CATEGORY.prescribe
			&& $(tr).find("td:eq(10)").text() == '门诊西药房') {
		showWithHtmlTemplate("print_prescribe_xiyaofang", print_data, page);
	}else if(listType == CHUZHI_CATEGORY.prescribe
			&& $(tr).find("td:eq(10)").text() == '中心摆药室') {
		showWithHtmlTemplate("print_prescribe_zxbys", print_data, page);
		...
		if(listType==CHUZHI_CATEGORY.Qg_operation)
			page.children('h2').text('屈光手术中心项目申请单');
		...
		if($("#printZd") != undefined && $("#printZd") != null && $("#zdTran") != undefined && $("#zdTran") != null){
			$("#zdTran").click(function(){
				var zdprint = $("#printZd").text();
				$("#printZd").html("");
				$("<input type='text' />").val(zdprint).focus().blur(function(){
					var tmpzdprint = $(this).val();
					$("#printZd").text(tmpzdprint);
				}).appendTo($("#printZd"));
			});
		}
		return page;
...
function getChuzhiText(){
	...
	if(data!=null){
		if(data.yyrq){
			...
			text+="<br />请于"+/* formatDate(data.yyrq.time) */value+"来本院复查。";
		}
		text+=data.zhuyi?("<br />"+(data.zhuyi)+"。"):"";
	}
	return text;
}
...
function getZKJCText(){
	...
	//得到txt同时保存到his
	getJSONData(saveZkjcToHis,{zkjc:txt,jiuzhenId:currentVisit.id},'POST');
	return txt;
...
function printOrders(combo){
	...
	if(currentVisit.haoma.toUpperCase().indexOf("OIMS")!=-1&&listType==CHUZHI_CATEGORY.labTest){
		$.oimsAlert("<p style='text-align:left; text-indent:2em'>因未找到患者的挂号信息，系统仅提供病历保存功能。请在医院系统中补开化验项目！</p>");
		return;
	}
	var page = null;//门诊西药房与公用
	var page1 = null;//便民药房
	var pagezxbys = null;//中心摆药室
	//筛选检查项目的打印数量ziduan
	...
	if(combo&&(combo=="choose" || combo=="yqchoose")){
		jifeiFlagPrint = table.children("tbody").children("tr.selected").length>0;
		$.each(table.children("tbody").children("tr.selected"), function(i,n){
			if(!$(n).hasClass("yijifei")){
				jifeiFlagPrint = false;
				return false;
			}
		});

	} else {
		jifeiFlagPrint = table.find("tr.yijifei").length==table.children("tbody").children("tr").length-1;
	}
	...
	$.each(table.find("tr"),function(i,tr){
		...
		if(combo&&(combo=="choose" || combo=="yqchoose")){//添加院前打印
				...
				if(!t.length||listType==CHUZHI_CATEGORY.otherExam){
					//如果没有这个部门的对应的page
						if(listType==CHUZHI_CATEGORY.otherExam){
							...
							if($(tr).find("td:eq(10)").text() == "中心摆药室"){
								if(pagezxbys==null){
									pagezxbys = getPageTag(listType,tr);
									$("<div />").css("clear","both").prependTo(pagezxbys);
								}
								else
									pagezxbys=pagezxbys.clone().appendTo(win);
							}else{
								if(page==null){
									page = getPageTag(listType,tr);
									$("<div />").css("clear","both").prependTo(page);
						...
						if(listType==CHUZHI_CATEGORY.otherExam){
								...
								var bgsObj = null;
								if (jcxmId == 285){
									var bgsId = $('#jctsDiv_285_14').find('select[name=zhixingkeshiSelected]').find('option:selected').val();
									bgsObj = getJSONData('/publish/bangongshi/getBanGongShiById.htm',{id:bgsId},'POST').obj;
								}else {
									bgsObj = getJSONData('/publish/bangongshi/getBanGongShiById.htm',{id:dept},'POST').obj;
								}

								var bgsbm=bgsObj.bgsBm;
								if(bgsbm=='4102'||bgsbm=='410201'||bgsbm=='410202'||bgsbm=='410203'||bgsbm=='410204'||bgsbm=='410205'||bgsbm=='410206'||bgsbm=='410230'){
									if(!tag.find('div.fsknote').length){
										$('<div  style="height:'+(296-8-32-$('table.print_sheet_'+dept).height())+'px;"/>').attr("id","fskdiv_"+dept).appendTo(tag);
										var div=$('<div  class="fsknote" style="height:32px"/>').appendTo(tag);
								...
								//如果检查项目是 （门诊）十二导心电同步分析检查（100%）或 肺功能三个检查则再打印单上显示检查科室的位置
								if (jcxmId == 1002145 || jcxmId == 1002432 || jcxmId == 1002433 || jcxmId == 1002434){
									if(!tag.find('div.fsknote').length){
										$('<div  style="height:'+(296-8-32-$('table.print_sheet_'+dept).height())+'px;"/>').attr("id","fskdiv_"+dept).appendTo(tag);
										var div=$('<div  class="fsknote" style="height:32px"/>').appendTo(tag);
										div.css({'font-size':'13px'});
		...
		if(listType==CHUZHI_CATEGORY.eyeExam||listType==CHUZHI_CATEGORY.treat||listType==CHUZHI_CATEGORY.Qg_operation||listType==CHUZHI_CATEGORY.operation){
			...
			if(listType==CHUZHI_CATEGORY.prescribe&&$(tr).find("td:eq(10)").text()=='中心摆药室'){
				var s = $("#totalPricezx");
				var totalPrice = (s.length && s.text().length)?parseFloat(s.text()):0;
				var tds = $(tr).children("td");
				for (var n = 0; n < tds.length; n++) {
					var td = tds[n];
						...
						var input = $(td).children("input");
						if(input.length){
							if(listType==CHUZHI_CATEGORY.prescribe&&n==8){
								td0.text(input.val()+$(td).text());
							}else{
								td0.text(input.val());
							}
				...
				$("#totalPricezx").text(totalPrice.toFixed(2));
			}else{
				var s = $("#totalPrice");
				var totalPrice = (s.length && s.text().length)?parseFloat(s.text()):0;
				var tds = $(tr).children("td");
	...
	function getPageTag(listType,tr){
		...
		var print_data={
				...
				zhenduan:zhenduan,
				kdsj:getDatetime(),
				jcxm: $(tr).find("td:eq(0)").text(),
				kdys:parent.window.currentStaff.xingming,
				chargeType:currentPatient.charge_type,
					job:currentPatient.job,
				keshi:"眼科门诊"};

		var yqStr = "";//院前打印需要不同模板
		if(combo&&combo=="yqchoose"){
			yqStr = "yq";
		}
	if(listType==CHUZHI_CATEGORY.otherExam)
		showWithHtmlTemplate(yqStr+"print_otherExam", print_data, page);
	 else if (listType == CHUZHI_CATEGORY.labTest){
		showWithHtmlTemplate(yqStr+"print_labTest", print_data, page);
	}else if (listType == CHUZHI_CATEGORY.prescribe) {
		if( ($(tr).find("td:eq(10)").text() == '便民药房'||$(tr).find("td:eq(10)").text() == '药品超市')){
			page.addClass("ypcs");
			if(print_data.zhenduan.length>30){
				print_data.zhenduan=print_data.zhenduan.substring(0,30)+"...";
			}
			showWithHtmlTemplate("print_prescribe_bianmin", print_data, page);
		}else if($(tr).find("td:eq(10)").text() == '中心摆药室'){//中心摆药室
			if(print_data.zhenduan.length>30){
				print_data.zhenduan=print_data.zhenduan.substring(0,30)+"...";
			}
			showWithHtmlTemplate("print_prescribe_zxbys", print_data, page);
		...
		showWithHtmlTemplate(yqStr+"print_eyeExam", print_data, page);
		if(listType==CHUZHI_CATEGORY.Qg_operation)
...
//			});
			...
			showBarcode(currentPatient.binglihao,page.find(".barcodeDiv"));
			if(fenzhenkaidan==1){
				var b_hospital=getJSONData(hospital_hz,{patientId:currentPatient.binglihao},'POST');
				if(b_hospital.obj){
					page.find('.qrcode').before("<div style='float: left;margin-left:15px;margin-top:5px;'><img src='/OIMS/images/hospitalTip.png' style='width: 50px;'></div>");
					page.find('.qrcode').css({'margin-left':'30px'});
					if($('div.qrcode').length){
						$('div.qrcode').next().remove();
						$('div.qrcode').remove();
					}
				}
			}
