			...
			if(listType==CHUZHI_CATEGORY.eyeExam||listType==CHUZHI_CATEGORY.treat||listType==CHUZHI_CATEGORY.Qg_operation||listType==CHUZHI_CATEGORY.operation){
				...
				$.each($('table.jcxmFushu'),function(i,n){
					if($(this).attr('id').split('_')[1]==jcxmId){
						...
						//名称
						if($(this).index()==0){
							if((parseInt(jcxmId)>=1000007&&parseInt(jcxmId)<=1000014)||parseInt(jcxmId)==1000025||parseInt(jcxmId)==2005||parseInt(jcxmId)==1000335||parseInt(jcxmId)==1000420||$.inArray(parseInt(jcxmId),print_jcxm_differ)!=-1){
...
//		$($('#careFrame').context).find('div').each(function(i,jele){
//			if(jele.attr('class')=='')
//		})
		
		var fu_jia_shuo_ming = "rnm" ;
		var jq_p_fjsm = $("<p />").html("附加说明："+fu_jia_shuo_ming) ;//jquery p元素的附加说明信息
		t.parent().append(jq_p_fjsm) ;
	});
	function getPageTag(listType,tr){
		importJS('/js/jquery.qrcode.js');
		var page = $("<div />").addClass("printPage").appendTo(win);
