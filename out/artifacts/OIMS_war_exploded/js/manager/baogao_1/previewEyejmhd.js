...
$(function() {
	...
	if (parameter_saveOrUpdateEyejmhd != null) {
		if ($("#zuoyan").length == 1)//激光类型
			$("#zuoyan").replaceWith($("<span/>").attr("id","zuoyan").html(
					parameter_saveOrUpdateEyejmhd.zuoyan));
		if ($("#youyan").length == 1)
			$("#youyan").replaceWith($("<span/>").attr("id","youyan").html(
					parameter_saveOrUpdateEyejmhd.youyan));
		if ($("#demo").length == 1)
			$("#demo").replaceWith($("<span/>").attr("id","demo").html(
					parameter_saveOrUpdateEyejmhd.demo));
		//申请医生
		if ($("#rep_doc").length == 1) 
			$("#rep_doc").replaceWith(
					$("<span/>").attr("id", "rep_doc").html(
							parameter_saveOrUpdateEyejmhd.repDoc_name));
		//检查医生
		if ($("#doctor").length == 1) 
			$("#doctor").replaceWith(
					$("<span/>").attr("id", "doctor").html(
							parameter_saveOrUpdateEyejmhd.doctor_name));
	}
	//添加条形码
...
function doPrintbaogao() {
	var url_saveOrUpdateEyejmhd = "/publish/Eyejmhd/saveOrUpdateEyejmhd.htm";// 角膜厚度报告保存或者修改
	$.extend(parameter_saveOrUpdateEyejmhd,{'baogaoState':oimsCategory.BAOGAO_STATE_TQ});
