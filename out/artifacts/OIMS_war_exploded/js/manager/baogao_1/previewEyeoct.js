...
$(function() {
	...
	if (parameter_saveOrUpdateEyeoct != null) {
		var val = parameter_saveOrUpdateEyeoct.memo;
		while(val.indexOf(" ")!=-1)
			val = val.replace(" ","&nbsp;&nbsp;");
		val = val.replace(/\r|\n/g,"<br />");
		if ($("#memo").length == 1)
		...
		if ($("#rep_doc").length == 1) 
			$("#rep_doc").replaceWith(
					$("<span/>").attr("id", "rep_doc").html(
							parameter_saveOrUpdateEyeoct.rep_doc_name));
		//检查医生
		if ($("#doctor").length == 1) 
			$("#doctor").replaceWith(
					$("<span/>").attr("id", "doctor").html(
							parameter_saveOrUpdateEyeoct.doctor_name));
		$("#div_controller").css("display","none");
	...
	var div_buttonsytle1 = $("<div/>").attr("id", "div_buttonsytle1").attr(
			...
			"width: 745px;margin: 0px auto;");// 操作按钮div
...
function doPrintbaogao() {
	var url_saveOrUpdateEyeoct = "/publish/Eyeoct/saveOrUpdateEyeoct.htm";
	$.extend(parameter_saveOrUpdateEyeoct,{'baogaoState':oimsCategory.BAOGAO_STATE_TQ});
	...
	if (data_saveOrUpdateEyeoct.state) {
		if(parameter_saveOrUpdateEyeoct.xmzl == '1'){
