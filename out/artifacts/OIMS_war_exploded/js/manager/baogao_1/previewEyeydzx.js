...
$(function() {
	...
	if (parameter_saveOrUpdateEyeydzx != null) {
		var val = parameter_saveOrUpdateEyeydzx.memo;
		while(val.indexOf(" ")!=-1)
			val = val.replace(" ","&nbsp;&nbsp;");
		val = val.replace(/\r|\n/g,"<br />");
		if ($("#memo").length == 1) 
		...
		if ($("#rep_doc").length == 1) 
			$("#rep_doc").replaceWith(
					$("<span/>").attr("id", "rep_doc").html(
							parameter_saveOrUpdateEyeydzx.rep_doc_name));
	//检查医生
		if ($("#doctor").length == 1) 
			$("#doctor").replaceWith(
					$("<span/>").attr("id", "doctor").html(
							parameter_saveOrUpdateEyeydzx.doctor_name));
...
// 打印页面(整理)
function doPrint() {
	var url_saveOrUpdateEyeydzx = "/publish/Eyeydzx/saveOrUpdateEyeydzx.htm";
