...
//	}
	...
	/** ********************************敏感度信息********************************** */
	var url_selectEyeminganduByEyemingandu = "/publish/Eyemingandu/selectEyeminganduByEyemingandu.htm";// 根据敏感度报告对象查询符合条件的敏感度报告对象
	var data_selectEyeminganduByEyemingandu = getJSONData(
			url_selectEyeminganduByEyemingandu, {
				jcdId : dataObjects_choice[0].jcdid,
				tag : Math.random()
			}, "post");
	if (data_selectEyeminganduByEyemingandu.state) {
//		showReportPictures();// 显示报告图片
		var eyemingandu = data_selectEyeminganduByEyemingandu.obj;
		if (eyemingandu != null) {
			if ($("#memo").length == 1)
				$("#memo").val(eyemingandu.memo);
			if ($("#cli_date").length == 1)
				$("#cli_date").html(eyemingandu.cli_date);// 报告日期
			if ($("#rep_doc").length == 1)
				$("#rep_doc").val(eyemingandu.rep_doc);// 申请医生
			if ($("#doctor").length == 1)
				$("#doctor").val(eyemingandu.doctor);// 检查医生
			if (data_outBaogaoHelp.obj.reportDate != eyemingandu.cli_date) {
				utilTool().fdisabled($("#div_reportresult"));// 全部只读
				$("#a_updatebaogao").attr("disabled", "disabled");
				$("#a_updatebaogao").unbind("click", saveOrUpdateEyemingandu);
			} 
//			else {
//				initPictureOperate();
//			}
...
/** **************************************图片相关操作结束**************************************** */
// 敏感度检查报告保存
function saveOrUpdateEyemingandu() {
	var oValidataData = {
			nullValidataData : {
				'memo' : '检查结果为空'
			}
...
//	}
	...
	var parameter_saveOrUpdateEyemingandu = {
//		path_pictures : path_pictures,
	...
	var url_saveOrUpdateEyemingandu = "/publish/Eyemingandu/saveOrUpdateEyeMingandu.htm";// 敏感度检查报告保存
	var data_saveOrUpdateEyemingandu = getJSONData(url_saveOrUpdateEyemingandu,
			parameter_saveOrUpdateEyemingandu, "post");
	if (data_saveOrUpdateEyemingandu.state)
		$.oimsSucc("敏感度检查报告保存成功", function() {

		});
	else
		$.oimsError("敏感度检查报告保存失败", function() {

		});
}

// 敏感度检查报告打印预览
function previewEyemingandu() {
	var oValidataData = {
			nullValidataData : {
				'memo' : '检查结果为空'
			}
...
//		path_pictures : path_pictures,
	...
	parameter_saveOrUpdateEyemingandu = JSON.stringify(
			parameter_saveOrUpdateEyemingandu)
	...
	html_baogao += "<script src='" + contextPath
			+ "/js/manager/baogao/previewEyemingandu.js'></script>";
	html_baogao += "<script type='text/javascript'>";
	html_baogao += "var parameter_saveOrUpdateEyemingandu="
			+ parameter_saveOrUpdateEyemingandu + ";";
	html_baogao += "</script>";
	html_baogao += "</head>";
