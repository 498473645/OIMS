...
function addTimeTag(dataO){
	...
	if(dataO[0].jcxmIds==json_jcxm.ktYanguang){//扩瞳验光要包含小瞳验光以及小瞳验光+老光
		//小瞳验光
		...
		//小瞳验光+老光
		var BaoGaoRelationTimeTag_data_xtlg=getJSONData("/publish/baogao/getBaoGaoRelationTimeTag.htm",{
			tag : Math.random(),
			binglihao:dataO[0].binglihao,
			jcxmId:json_jcxm.xtYanguangLg
		},"POST");
		if(BaoGaoRelationTimeTag_data_xtlg.obj!=null){
			BaoGaoRelationTimeTag_data.obj=BaoGaoRelationTimeTag_data.obj.concat(BaoGaoRelationTimeTag_data_xtlg.obj);
			BaoGaoRelationTimeTag_data.obj.sort(function(v1,v2){
				if(v1.jcdId>v2.jcdId){
					return -1;
				}
	...
	}else if(dataO[0].jcxmIds==json_jcxm.xtYanguang){//小瞳验光要包含扩瞳验光以及小瞳验光+老光
	   //扩瞳验光
	...
	}else if(dataO[0].jcxmIds==json_jcxm.xtYanguangLg){//小瞳验光+老光要包含扩瞳验光以及小瞳验光
    	//小瞳验光
		var BaoGaoRelationTimeTag_data_xt=getJSONData("/publish/baogao/getBaoGaoRelationTimeTag.htm",{
			tag : Math.random(),
			binglihao:dataO[0].binglihao,
...
//				bumenId : jcd.jcksId,// 检查科室ID
	...
	if(tag==null||tag==undefined){
		...
		//luke change tmp 加上id属性无影响 暂时不处理
...
function reportController(jcxmIds) {
	if (jcxmIds == json_jcxm.anshiyingjiancha||jcxmIds == json_jcxm.gfs) {// 暗适应
		...
		// $("#a_closebaogao").bind("click", renovate_BaoGaoNotIs);
			...
			|| jcxmIds == json_jcxm.ydzx_jfw
			|| jcxmIds == json_jcxm.ydzx
			|| jcxmIds == json_jcxm.zfygFFA
			|| jcxmIds==json_jcxm.zfyg_jfw
...
//	} else 
		...
		if ($("#a_savebaogao").length == 1)
		    ...
		    ||jcxmIds == json_jcxm.xtYanguangLg
	...
	} else if (jcxmIds == json_jcxm.yanweiyanji || jcxmIds == json_jcxm.yanwaiji) {// 眼位眼肌
		initData_eyeywyj();
		if ($("#a_printreport").length == 1)
...
function reportControllerWenZi(jcxmIds, baogaoMoban_jcd) {
	...
	} else if (jcxmIds == json_jcxm.yanweiyanji || jcxmIds ==  json_jcxm.yanwaiji) { // 眼位眼肌检查
		init_Baogao_Moban(baogaoMoban_jcd);
		initData_eyeywyj();
		$("#a_printreport").bind("click", previewEyeywyj);
		$("#a_savebaogao").bind("click", function() {
