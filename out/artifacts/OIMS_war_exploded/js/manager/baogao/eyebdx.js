...
//	}
	...
	//根bdx报告对象查询符合条件的bdx报告对象
	var url_selectEyebdxByEyebdx = "/publish/Eyebdx/selectEyebdxByEyebdx.htm";
	var data_selectEyebdxByEyebdx = getJSONData(
			url_selectEyebdxByEyebdx, {
				jcdId : dataObjects_choice[0].jcdid,
				tag : Math.random()
			}, "post");
	if (data_selectEyebdxByEyebdx.state) {
		var eyebdx = data_selectEyebdxByEyebdx.obj;
		if (eyebdx != null) {
			if ($("#ls_r").length == 1)
				$("#ls_r").val(eyebdx.ls_r);// 裸视
			if ($("#ls_l").length == 1)
				$("#ls_l").val(eyebdx.ls_l);//裸视
			if ($("#dj_r").length == 1)
				$("#dj_r").val(eyebdx.dj_r);// 戴镜
			if ($("#dj_l").length == 1)
				$("#dj_l").val(eyebdx.dj_l);// 戴镜
			if ($("#cli_date").length == 1)
				$("#cli_date").html(eyebdx.cliDate);// 报告日期
			if ($("#rep_doc").length == 1)
				$("#rep_doc").val(eyebdx.repDoc);// 申请医生
			if ($("#doctor").length == 1)
				$("#doctor").val(eyebdx.doctor);// 检查医生
			if (data_outBaogaoHelp.obj.reportDate != eyebdx.cliDate) {
				utilTool().fdisabled($("#div_reportresult"));// 全部只读
				$("#a_updatebaogao").attr("disabled", "disabled");
				$("#a_updatebaogao").unbind("click", saveOrUpdateEyebdx);
			}
		}
	}
}
...
// bdx检查报告保存
function saveOrUpdateEyebdx(state) {
	var oValidataData = {
		nullValidataData : {
	...
	var parameter_saveOrUpdateEyebdx = {
		jcdId : jcdId,// 检查单ID
		ls_r : ls_r,
		ls_l : ls_l,
		dj_r : dj_r,
	...
	var url_saveOrUpdateEyebdx = "/publish/Eyebdx/saveOrUpdateEyebdx.htm";// bdx检查报告保存
	...
	if (data_saveOrUpdateEyebdx.state)
		...
		$.oimsError("不等像检查报告保存失败", function() {
			if (state!= undefined &&state == 1) {
				importJS("/js/manager/jiancha/eyeReportCommon.js"); // 加载关闭Dialog的窗口时初始化待检患者界面
				closeReporeDialog();
			}
...
function previewEyebdx() {
	...
	parameter_saveOrUpdateEyebdx = JSON.stringify(
			parameter_saveOrUpdateEyebdx)
	...
	html_baogao += "<script src='" + contextPath
			+ "/js/manager/baogao/previewEyebdx.js'></script>";
	html_baogao += "<script type='text/javascript'>";
	html_baogao += "var parameter_saveOrUpdateEyebdx="
			+ parameter_saveOrUpdateEyebdx + ";";
	html_baogao += "</script>";
	html_baogao += "</head>";
