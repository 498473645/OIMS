...
var eye_data_eyejmdxt_check_doc = [ {
	value_check_doc : '',
	key_check_doc : ''
}, {
	value_check_doc : '暗适应',
...
// 暗适应检查报告必要的数据初始化
function initData_eyejmdxt_jiancha() {
	// 检查子类下拉框赋值
	if ($("#check_doc").length == 1) {
		for (var i = 0; i < eye_data_eyejmdxt_check_doc.length; i++)
			$(
					"<option value=\""
							+ eye_data_eyejmdxt_check_doc[i].value_check_doc
							+ "\">"
							+ eye_data_eyejmdxt_check_doc[i].key_check_doc
...
// 暗适应检查报告保存
function saveOrUpdateEyejmdxt_jiancha() {
	var oValidataData = {
		nullValidataData : {
			'check_doc' : '检查子类为空',
			'jm_ql' : '检查结果为空',
...
// 暗适应检查报告打印预览关闭
function previewEyejmdxt_jiancha() {
	var oValidataData = {
		nullValidataData : {
			'check_doc' : '检查子类为空',
			'jm_ql' : '检查结果为空',
