...
var qg_jbxx = null;//屈光 患者基本信息
var qg_ygdata = null;//屈光 员工信息
var qg_yglist = null;//屈光 员工列表
var firstJz = null;//首次就诊记录
var tag_click = 0;
...
function _emr_preview_qg(id){
	...
	//这里 流程到哪里 默认就打开哪个标签start↓
	var lc_name = $("#lc_name").text();
	$.each($("#div_show_4_2_1").find("div"),function(){
		if($(this).hasClass("tab_show") || $(this).hasClass("tab_hide")){
			var spatext = $(this).find("span").eq(0).text();
			if(spatext == lc_name){
				$(this).click();
			}
		}
	});
	//这里 流程到哪里 默认就打开哪个标签end↑
}
...
function getJbxx_qg(blh){
	if(qg_jbxx != null && qg_jbxx != undefined){//如果 已经有基本信息了 直接拿出来用
		return qg_jbxx;
	}else{
		var jz_first = null;
		if(firstJz != null && firstJz != undefined){
			jz_first = firstJz;
		}else{
			jz_first = getJiuzhen_qg(blh,"first");
			firstJz = jz_first;
		}
...
//获取员工信息
function getUser(){
	if(qg_ygdata != null && qg_ygdata != undefined){//若已经存在 直接使用
		return qg_ygdata;
	}else{
...
function qulc_set(){
	var blh = $("#patientInfo").children("span").first().text();
	var data = getJSONData(EMR_QGLC_GET_WWC_URL, {
		blh : blh,
		tag : Math.random()
	}, "POST");
	var lc_name = "";
	if(data!=null){
		$("#lc_id").text(data.id);	
		lc_name = QG_LC_DATA[data.state].title;
	}else{
		$("#lc_id").text("无");
		lc_name = QG_LC_DATA[0].title; 
	}
...
function fenlei_lev2(){
	...
	var lcName = $("#lc_name").text();
	var flaglc = true;
	$.each($(".tab_show"),function(k,m){
		var tb_text = $(m).find("span").eq(0).text();//选择标签中的文字
		if(tb_text == "手术记录"){
			flaglc = false;
		}
	});
	if(lcName != "手术记录" && flaglc){
		//改变手术方式，刷新病历页面
		_emr_show_bl();
	}
...
function zfz_bl(){
	...
	/*$("#div_show_4_2_2").html("");
	$("<div />").attr("id","show_bl").appendTo($("#div_show_4_2_2"));
	var f = showFormByHtmlTemplate($("#show_bl"), "/emr/template/html/emr_zunfenzi.html", {
		formMethod:null,
		beforeSubmit:function(){},
...
function setSsfs(){
	var lc_id = $("#lc_id").text();
	var lcObj =  getJSONData(EMR_QGLC_GET_URL, {
		id : lc_id,
		tag : Math.random()
	}, "POST").obj;
...
function view_qg(id){
	...
	if(tag.length){
		...
		var tag4_2_2 = $("<div/>").attr("id","div_show_4_2_2").attr("style","width:"+(qgblyl_width-40)+"px;height:"+(h-30-28)+"px;background:;overflow:auto;").appendTo(tag4_2);
		//读病例模板
		_emr_show_bl();
		//页签
		var tag = $(tag4_2_1);
		if(!tag.children().length){
			...
			$.each(data,function(i,d){
				...
				div.addClass(className).click(function(){//这里 设置了 默认选择病历
					if(d.id=='30021'){
						$("#tag_ml").show();
					}else{
						$("#tag_ml").hide();
...
function setValue_qg(){
	...
	if(ssfs!="--请选择--"){
		...
		if(url!="" && lc_id!=null && lc_id!='无'){
			var data = null;
			if(sel_tag!="手术记录"){
				data = getJSONData(url,{lc_id:lc_id},"POST").obj;
			}else{
				var ssjl_yanbie = $("input[name='yanbie']:checked").val();
				data = getJSONData(url,{lc_id:lc_id,yanbie:ssjl_yanbie,tag:Math.random()},"POST").obj;
			}
			if(data!=null){
				if(sel_tag=="病历"){
					if(ssfs=="准分子" || ssfs=="晶体植入"){
 ...
 * 读模板 方法
 */
function blmb1(mobanId){//修改
	var blmb = "";
	var templateUrl = "/emr/template/html/emr_qg_"+mobanId+".html";
	$.ajax({url:contextPath+templateUrl+"?tag="+Math.random(), async : false, type : "GET", success : function(t){blmb=t;}});
	if(blmb == "" || blmb == null || blmb == undefined){
...
function findQgYgList(){
	if(qg_yglist != null && qg_yglist != undefined){
		return qg_yglist;
	}else{
...
function jt_ssjl_get(){
	...
	//开始时间
	var kssj = $("#kssj").val()==""?new Date(null):new Date($("#kssj").val());
	var jssj = $("#jssj").val()==""?new Date(null):new Date($("#jssj").val());
	//手术名称
...
function tbl_yy_add(){
	...
	var tab_add = "<table id='ssjlAdd' cellpadding='0' cellspacing='0' width='100%' style='border-top:1px solid rgb(51,51,51);border-left:1px solid rgb(51,51,51);'>" +
					...
					"</tr>" +
					"<tr>" +
						"<td align='center' class='td_border' rowspan='2'>预约项目：</td>" +
					"<td align='center' class='td_border' colspan='3' align='center'>" +
						"<input type='radio' id='yyxm1' name='yyxm'/>&nbsp;&nbsp;LASIK系列&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
					...
					"<td align='center' class='td_border' colspan='4' align='center'>" +
						"<input type='radio' id='yyxm2' name='yyxm'/>&nbsp;&nbsp;LASEK 系列&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
					...
					"<tr>"+
					"<td align='center' class='td_border' colspan='3' align='center'>" +
						"<input type='radio' id='yyxm3' name='yyxm'/>&nbsp;&nbsp;飞 秒 系 列&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
					...
					"</td>" +
					"<td align='center' class='td_border' colspan='4' align='center'>" +
						"<input type='radio' id='yyxm4' name='yyxm'/>&nbsp;&nbsp;ICL/TICL/PRL&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
				...
				"<tr>"+
				"<td align='center' class='td_border' colspan='3' align='center'>" +
					"<input type='radio' id='yyxm5' name='yyxm'/>&nbsp;&nbsp;Smart-TPRK&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
					"&nbsp;&nbsp;或&nbsp;&nbsp;<input type='text' id='ssfy5h' style='width:50px;'/>  元"+
				"</td>" +
				"<td align='center' class='td_border' colspan='4' align='center'>" +
					"<input type='radio' id='yyxm6' name='yyxm'/>&nbsp;&nbsp;全飞秒&nbsp;&nbsp;&nbsp;&nbsp;手术费用：&nbsp;&nbsp;"+
...
function yy_printArea(){
	...
	var sj = temp.yysj;
	//sj = parseDatetime(sj);//如果没有转换  需要转换一下
	if(sj == null){
		return "";
	}
...
//晶体手术记录 修改眼别
function ssfs_change_yb(){
	var sg = $("#sg").val();
	var tz = $("#tz").val();
	var yanbie = $("input[name='yanbie']:checked").val();
	_emr_show_ssjl();
	if(sg != ""){
		$("#sg").val(sg);
	}
	...
	if(yanbie != "" && yanbie != undefined){
		$.each($("input[name='yanbie']"),function(){
			var v = $(this).val();
			if(v == yanbie){
