...
function qg_login() {
	...
	$("#opencontent_div #binglihaoSearchXX").click(
			function() {
				...
				if (binglihao != null && binglihao != "") {
					...
					if (local_hzxx.state && local_hzxx.obj != null) {
						fz_showPatientVisits(local_hzxx.obj);
					} else {
						...
						if (his_hzxx.state && his_hzxx.obj != null) {
							fz_showPatientVisits(his_hzxx.obj);
						} else {
							$.alert("查无此人，请确认ID号是否正确！");
						}
					}
				}
			});
...
// 查询本地数据库
function fz_sel_hzxx_local() {
	var binglihao = $("#binglihao").val();
	if (binglihao != null && binglihao != "") {
		var fz_sel_hzxx_data = getJSONData(
				huanZheFenZhen_selectHuanXhe_url, {
					binglihao : binglihao,
					tag : Math.random()
				}, "POST");
		if (fz_sel_hzxx_data.state && fz_sel_hzxx_data.obj != null){
			fz_ToDayJiuZhen(fz_sel_hzxx_data);
		}else{
			fz_selectHuanXhe_his(binglihao);
		}
	} else {
		$.oimsAlert("请输入ID号！");
		//showPatientListToday(); // 显示患者信息列表
...
/** ***************************显示患者信息****************************** */
function fz_showPatientVisits(hzxx) {
...
// 查询his数据库
function fz_selectHuanXhe_his(binglihao) {
	...
	if (huanZheFenZhen_selectHuanXhe_his_data.state
		...
		$.oimsConfirm({
			...
			fz_fenzhen(huanZheFenZhen_selectHuanXhe_his_data);
		}, function() {
			// alert("显示患者信息列表");
			//showPatientListToday(); // 显示患者信息列表
		});
...
// 根据病历号接诊该患者
function toQgbl(binglihao) { // 病历号或者是就诊id
	...
	iframe[0].src = contextPath+"/emr/emr_qg.jsp?binglihao=" + binglihao;
}
