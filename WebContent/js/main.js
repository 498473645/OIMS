...
var userData;
var currentStaff = JSON.parse($.cookie("staff"));// 当前员工
var config,currentBtns;
var task = {};
var skinPath = $.cookie("skinPath") == undefined
		|| $.cookie("skinPath") == null ? "green" : $.cookie("skinPath");
...
function getUserData() {
	var result = getJSONData("/publish/user/getUserData.htm", {
		tag : Math.random()
	});
...
function showWelcome() {
	...
	if (ygdata.state) {
		...
		if (yg.photo != null && yg.photo != "") {
			$("<img/>").attr("src", contextPath + yg.photo).width(38)
					.height(38).appendTo(span);
		} else {
			$("<img/>").attr("src", "../ygPhoto/head.png").width(38).height(38)
					.appendTo(span);
		}
...
function showTongzhi(tag){
	if(tag==undefined || tag==null) tag = $("<div />").css("overflow","auto").addClass("tongzhiShowDiv").prependTo("#right");
	var w = tag.parent().width()/2-8;
	var h = tag.parent().height()/2-8;
	var p = tag.offset();
	tag.width(w).height(h).css({top:p.top+2,left:p.left+2});
	$("<div />").addClass("tongzhiShowDivBg").width(w).height(h).css({top:p.top+2,left:p.left+2}).prependTo("#right");
	$("<h1 />").text("通知").appendTo(tag);
	var ul = $("<ul />").appendTo(tag);
	var re = getJSONData(findTongzhiUrl,{max:10,tag:Math.random()},"POST");
	$.each(re.obj,function(i,d){
		...
		$("<span />").html(formatDateTime(d.insertTime.time)).appendTo(li);
...
function showThesisPublished(tag){
	if(tag==undefined || tag==null) tag = $("<div />").css("overflow","auto").addClass("tongzhiShowDiv").addClass("articleShowDiv").prependTo("#right");
	var w = tag.parent().width()/2-8;
	var h = tag.parent().height()/2-8;
	var p = tag.offset();
	tag.width(w).height(h).css({top:p.top+2,left:p.left+6+w});
	$("<div />").addClass("tongzhiShowDivBg").width(w).height(h).css({top:p.top+2,left:p.left+6+w}).prependTo("#right");
	$("<h1 />").text("论文").appendTo(tag);
	var ul = $("<ul />").appendTo(tag);
	var re = getJSONData("/publish/yuangong/findJianLiByConditon.htm",{currentPage:1,pageSize:10,type:'fblw'},"POST");
	$.each(re.list,function(i,d){
		var li = $("<li />").appendTo(ul);
		$("<span />").text("["+d.detailKind+"]").appendTo(li);
		$("<span />").text("["+d.detailType+"]").appendTo(li);
		var a = $("<a />").attr("href","javascript:void(0);").click(function(){
			if(d.fujian)window.open(d.fujian);
		}).text(d.project_name).appendTo(li);
		$("<span />").html(formatDateTime(d.finalTime.time)).appendTo(li);
	});
}
...
function showOperationPlanList(tag){
	if(tag==undefined || tag==null) tag = $("<div />").css("overflow","auto").addClass("operationPlan").addClass("tongzhiShowDiv").prependTo("#right");
	var w = tag.parent().width()-12;
	var h = tag.parent().height()/2-8;
	var p = tag.offset();
	tag.width(w).height(h).css({top:p.top+4+h,left:p.left+2});
	
	$("<div />").addClass("tongzhiShowDivBg").width(w).height(h).css({top:p.top+4+h,left:p.left}).prependTo("#right");
	...
	var page=1;
	while(true){
		var re = getJSONData("/publish/shoushu/findOperationList.htm",{tag:Math.random(),currentPage:page,pageSize:10,process:2},"POST");
		$.each(re.list,function(i,d){
			d = $.extend(d,{sex:(d.sex?"男":"女"),age:getAge(d.birthday.time),appointmentTime:formatDateTime(d.appointmentTime.time)})
			...
			var a = $("<a />").attr("href","javascript:void(0);").text(s).appendTo(td);
...
//			console.log("re:"+d.read);
	/*		if(!d.read){
				var span = $("<span />").addClass("new").text("NEW").appendTo(td);
				a.click(function(){
					span.remove();
				});
			}*/
...
function showThisOperationPlan(d){
	...
	var template = common_getHtmlTemplate(contextPath+"/js/manager/shoushu/template/operationAnpaiShow.html?tag="+Math.random());
	$("<div />").attr("id","operationView"+d.id).append(template).oimsDialog({
		icon:"view",
		title:"手术安排查看",
		locked:true,
		width:610,
	...
	d = $.extend(d,{category:category,operationNames:operationNames,categoryName:categoryName,isolation:isolation,operationSize:operationSize,anesthesiaName:a,opertionRoomName:room,levelFlag:level});
...
$(document).ready(function() {
	...
	//$.oimsAlert("<p style='font-size:18px;'>通知：医生工作站处置下的检查项目与处方的选取与删除都改为双击,有问题咨询765389</p>");
	showTongzhi();
	showThesisPublished();
...
$("#content .left .menu .modulertitle").click(function(){
	if(navigator.appName.indexOf('Microsoft Internet Explorer')!=-1){
		var parentWindow=window.open('index.htm','','fullscreen=0,channelmode=1,resizable=yes');//channelmode=yes,fullscreen=1,resizable=1,location=no,menubar=no,status=no,titlebar=no
		window.opener=null;
		window.open("","_self");
		window.close();
		return;
