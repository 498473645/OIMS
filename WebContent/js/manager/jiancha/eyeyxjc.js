//隐斜检查初始化数据
function initData_eyeyxjc_jiancha() {
	// 申请医生下拉框赋值
	if ($("#rep_doc").length == 1) {
	...
	if (data_selectEyeyxjc.state) {
		...
		if (eyeyxjc != null) {
			...
			if ($("#cli_date").length == 1)
				$("#cli_date").val(eyeyxjc.cli_date); // 检查时间
			if ($("#rep_doc").length == 1)
			...
			if (data_outBaogaoHelp.obj.reportDate != eyeyxjc.cli_date) {
				...
				$("#a_updatebaogao").unbind("click", saveOrUpdateEyexjc);
			}
		}
	}
...
// 保存或修改隐斜检查
function saveOrUpdateEyeyxjc_jiancha() {
	var oValidataData = {
		nullValidataData : {
			'jcfs' : '检查方式为空',
			'result' : '检查结果为空',
...
// 隐斜检查打印预览
function previewEyeyxjc_jiancha() {
	var oValidataData = {
		nullValidataData : {
			'jcfs' : '检查方式为空',
			'result' : '检查结果为空',
	...
	var rep_doc = $("#rep_doc option:selected").val();// 申请医生
	var doctor = $("#doctor option:selected").val();// 检查医生
	var rep_doc = $("#rep_doc option:selected").text();// 申请医生
	var doctor = $("#doctor option:selected").text();// 检查医生
	var cli_date = $("#cli_date").html(); // 报告日期
	var parameter_saveOrUpdateEyeyxjc = {
			jcdId : jcdId,
			huanzhexinxi_id : huanzhexinxi_id,
	...
	parameter_saveOrUpdateEyeyxjc = JSON.stringify(parameter_saveOrUpdateEyeyxjc).replace(new RegExp("\"","gm"),"'");
	...
	html_baogao+="<link rel='stylesheet' type='text/css' href='"+contextPath+"/css/main.css' >";
	html_baogao+="<link rel='stylesheet' type='text/css' href='"+contextPath+"/style/green/css/green.css' >";
	html_baogao+="<link rel='stylesheet' type='text/css' href='"+contextPath+"/css/icon.css' >";
	html_baogao+="<link rel='stylesheet' type='text/css' href='"+contextPath+"/style/green/css/openWin.css' >";
	...
	html_baogao+="<script src='" + contextPath + "/js/manager/baogao/previewEyeyxjc.js'></script>";
	html_baogao+="<script type='text/javascript'>"+"var parameter_saveOrUpdateEyeyxjc = "+parameter_saveOrUpdateEyeyxjc+";";
	html_baogao+="</script>";
	html_baogao += "</head>";
