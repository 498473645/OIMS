...
function _emr_recordCompare(){
	...
	var selectRecords = $('#recordlistdiv').find('input[type="checkbox"]:checked');
	...
	var container = $('<div id="comparecontainer" class="emr_duibi00"/>').appendTo('body');
	...
	$.each(visits,function(){
		$('<td class="timetdw" style="width:'+width+'%"/>').text(this.date).appendTo(timeTr);
		date.push(this.date);
	});
	...
	/**诊断*/
	_emr_createResultCompareTable(contentDiv,data[3],width);
	/**处方*/
