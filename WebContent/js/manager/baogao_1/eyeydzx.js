...
//	}
	...
	/** ********************************眼前节照相信息********************************** */
	var url_selectEyeydzxByEyeydzx = "/publish/Eyeydzx/selectEyeydzxByEyeydzx.htm";// 根据眼底照相报告对象查询符合条件的眼底照相报告对象
	var data_selectEyeydzxByEyeydzx = getJSONData(
			url_selectEyeydzxByEyeydzx, {
				jcdId : dataObjects_choice[0].jcdid,
				tag : Math.random()
			}, "post");
	if (data_selectEyeydzxByEyeydzx.state) {
		showReportPictures();// 显示报告图片
		var eyeydzx = data_selectEyeydzxByEyeydzx.obj;
		if (eyeydzx != null) {
			if ($("#memo").length == 1)
				$("#memo").val(eyeydzx.memo);
			if ($("#cli_date").length == 1)
				$("#cli_date").html(eyeydzx.cli_date);// 报告日期
			if ($("#rep_doc").length == 1)
				$("#rep_doc").val(eyeydzx.rep_doc);// 申请医生
			if ($("#doctor").length == 1)
				$("#doctor").val(eyeydzx.doctor);// 检查医生
			if (data_outBaogaoHelp.obj.reportDate != eyeydzx.cli_date) {
				utilTool().fdisabled($("#div_reportresult"));// 全部只读
				$("#a_updatebaogao").attr("disabled", "disabled");
				$("#a_updatebaogao").unbind("click", saveOrUpdateEyeydzx);
			} else {
				initPictureOperate();
			}
		} else {
...
function saveOrUpdateEyeydzx() {
	...
	var parameter_saveOrUpdateEyeydzx = {
	...
	var url_saveOrUpdateEyeydzx = "/publish/Eyeydzx/saveOrUpdateEyeydzx.htm";// 眼底照相检查报告保存
	var data_saveOrUpdateEyeydzx = getJSONData(url_saveOrUpdateEyeydzx,
			parameter_saveOrUpdateEyeydzx, "post");
	if (data_saveOrUpdateEyeydzx.state)
		$.oimsSucc("眼底照相检查报告保存成功", function() {

		});
	else
		$.oimsError("眼底照相检查报告保存失败", function() {

		});
}

// 眼底照相检查报告打印预览
function previewEyeydzx() {
	var path_pictures = "";
	var operateDiv = $(".operateDiv");
	var array_div = $(operateDiv)[0].children;
	for ( var i = 0; i < array_div.length; i++) {
	...
	parameter_saveOrUpdateEyeydzx = JSON.stringify(
			parameter_saveOrUpdateEyeydzx)
	...
	html_baogao += "<script src='" + contextPath
			+ "/js/manager/baogao/previewEyeydzx.js'></script>";
	html_baogao += "<script type='text/javascript'>";
	html_baogao += "var parameter_saveOrUpdateEyeydzx="
			+ parameter_saveOrUpdateEyeydzx + ";";
	html_baogao += "</script>";
	html_baogao += "</head>";
