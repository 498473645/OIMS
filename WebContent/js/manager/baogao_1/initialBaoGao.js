//报告页面各元素赋值 (整理)
function initial_Baogao(BaogaoForm) {
	if ($("#suffererName").length == 1)// 患者姓名
		$("#suffererName").html(BaogaoForm.suffererName);
	if ($("#sex").length == 1)
		$("#sex").html(BaogaoForm.sex);// 患者性别
	if ($("#age").length == 1)
		$("#age").html(BaogaoForm.age);// 患者年龄
	if ($("#caseNumber").length == 1)
		$("#caseNumber").html(BaogaoForm.caseNumber);// 患者病历号
	if ($("#department").length == 1)
		$("#department").html(BaogaoForm.department);// 科别
	if ($("#eyeSex").length == 1)
		$("#eyeSex").html(BaogaoForm.eyeSex);// 眼别
	if ($("#ultraAudibleSoundNumber").length == 1)
		$("#ultraAudibleSoundNumber").html(BaogaoForm.ultraAudibleSoundNumber);// 超声号
	if ($("#cli_date").length == 1)
		$("#cli_date").html(BaogaoForm.reportDate);// 报告日期
	if ($("#reportDoctor").length == 1)
		$("#reportDoctor").html(BaogaoForm.reportDoctor);// 报告医生
	$("<option value=\"" + BaogaoForm.shengQingYiShengGongHao + "\">"
					+ (BaogaoForm.shengQingYiSheng=='admin'?'-':BaogaoForm.shengQingYiSheng) + "</option>").appendTo(
			"#rep_doc");
	if ($("#doctor").length == 1)
		$("#doctor").val(BaogaoForm.jianChaYiShengGongHao);// 检查医生

}


function printBaoGaoLodap(pageFormat){
	importJS("/js/LodopFuncs.js");
	var strHtml="<body>"+window.document.body.innerHTML+"</body>"; 
	LODOP = getLodop();  
	LODOP.PRINT_INIT("OIMS打印");
	LODOP.ADD_PRINT_HTM(0,0,"100%","100%",strHtml);
	LODOP.SET_PRINT_MODE("FULL_WIDTH_FOR_OVERFLOW",true);
	LODOP.SET_PRINT_MODE("FULL_HEIGHT_FOR_OVERFLOW",true); 
	LODOP.SET_PRINT_PAGESIZE(0,0,0,pageFormat);
	LODOP.PRINT();	
}
		...
		$
				.each(
						...
						function(i, Eyereportpicture) {
							var operateDiv = $(".operateDiv");
							var array_div = $(operateDiv)[0].children;
							if (array_div[Eyereportpicture.paixu] != undefined) {
								$(array_div[Eyereportpicture.paixu]).html("");
								var id_image = encryption();
								var clientWidth = array_div[Eyereportpicture.paixu].clientWidth;
								var clientHeight = array_div[Eyereportpicture.paixu].clientHeight;
								var url_getImageInfoByPath = "/publish/image/getImageInfoByPath.htm";
								var path_Image = Eyereportpicture.path_picture
										.substring(Eyereportpicture.path_picture
												.indexOf("/") + 1);
								...
								if (data_getImageInfoByPath.state) {
									var image_div = $("<img/>")
											...
											.attr(
													...
													Eyereportpicture.path_picture)
											.attr("class", "class_image_div");
									$(image_div).appendTo(array_div[Eyereportpicture.paixu]);
								}
							}
						});
	}
