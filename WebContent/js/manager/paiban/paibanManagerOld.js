...
function addPaiban(){
	...
	showPaibanCalendarForm(mTag);
//	paiban_saveOrUpdateForm(1);
}

function showPaibanCalendarForm(tag){
	var startDate = getFistDateOfMonth();
	$("<h1 />").text(startDate.getFullYear()+"年"+(startDate.getMonth()+1)+"月工作安排").appendTo(tag);
	var day = startDate.getDay();
	var endDate = getLastDateOfMonth();
	var table = $("<table />").appendTo(tag);
	var title = $("<tr />").appendTo(table);
	var ds = ["星期日","星期一","星期二","星期三","星期四","星期五","星期六"];
	for(var i=0; i<ds.length; i++){
		$("<th />").text(ds[i]).appendTo(title);
	}
	var tr = $("<tr />").appendTo(table);
	if(day>0){
		for(var i=0; i<day; i++){
	...
	while(date.getTime()<=endDate.getTime()){
		...
		$("<div />").css({width:"120px", height:"22px", "line-height":"22px", "text-align":"center", "font-weight":"bold"}).text(date.getDate()).appendTo(td);
		$("<div />").attr("id","date"+formatDate(date)).css({width:"120px",height:"50px", border:"1px solid #ddd"}).appendTo(td);
		if(date.getDay()==6)tr = $("<tr />").appendTo(table);
