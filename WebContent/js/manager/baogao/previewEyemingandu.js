...
$(function() {
	...
	if (parameter_saveOrUpdateEyemingandu != null) {
		var val = parameter_saveOrUpdateEyemingandu.memo;
		while(val.indexOf(" ")!=-1)
			val = val.replace(" ","&nbsp;&nbsp;");
		val = val.replace(/\r|\n/g,"<br />");
		if ($("#memo").length == 1) 
		...
		if ($("#rep_doc").length == 1) 
			$("#rep_doc").replaceWith(
					$("<span/>").attr("id", "rep_doc").html(
							parameter_saveOrUpdateEyemingandu.rep_doc_name));
	//检查医生
		if ($("#doctor").length == 1) 
			$("#doctor").replaceWith(
					$("<span/>").attr("id", "doctor").html(
							parameter_saveOrUpdateEyemingandu.doctor_name));
...
function doPrintbaogao() {
	var url_saveOrUpdateEyemingandu = "/publish/Eyemingandu/saveOrUpdateEyeMingandu.htm";
	$.extend(parameter_saveOrUpdateEyemingandu,{'baogaoState':oimsCategory.BAOGAO_STATE_TQ});
